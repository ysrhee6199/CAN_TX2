...
int open_read_port(const char *port)
{
    struct ifreq ifr;
    struct sockaddr_can addr;
    /* open socket */
    read_soc = socket(PF_CAN, SOCK_RAW, CAN_RAW);
    if(read_soc < 0)
    {
        printf("error!");
        return (-1);
    ...
    if (ioctl(read_soc, SIOCGIFINDEX, &ifr) < 0)
    {
        printf("error!");
        return (-1);
    }
    addr.can_ifindex = ifr.ifr_ifindex;
    fcntl(read_soc, F_SETFL, O_NONBLOCK);
    if (bind(read_soc, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    {
...
int write_soc;
int open_write_port(const char *port)
{
    struct ifreq ifr;
    struct sockaddr_can addr;
    /* open socket */
    write_soc = socket(PF_CAN, SOCK_RAW, CAN_RAW);
    if(write_soc < 0)
    {
        printf("error!");
        return (-1);
    ...
    addr.can_family = AF_CAN;
    strcpy(ifr.ifr_name, port);
    if (ioctl(write_soc, SIOCGIFINDEX, &ifr) < 0)
    {
        printf("error!");
        return (-1);
    }
    addr.can_ifindex = ifr.ifr_ifindex;
    fcntl(write_soc, F_SETFL, O_NONBLOCK);
    if (bind(write_soc, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    {
        printf("binding error!");
        return (-1);
    }
